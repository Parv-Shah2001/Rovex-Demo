...
Dependency Injection checks to enforce permission hierarchies (Supervisor, Sub-supervisor, Employee).
"""
...
class RBACChecker:
    """
    Enforces route-level access permissions based on role hierarchies.
    Roles are: supervisor, sub-supervisor, employee
    Hierarchy:
      - supervisor: can perform supervisor actions + sub-supervisor + employee actions
      - sub-supervisor: can perform sub-supervisor + employee actions
      - employee: can only perform employee actions
    """
    ...
    def __call__(self, current_user: Dict[str, Any] = Depends(get_current_user_from_cookie_or_header)) -> Dict[str, Any]:
        ...
        is not within the allowed_roles list.
        """
        user_role = current_user["role"]
        
        # Superuser/supervisor override
        if user_role == "supervisor":
            # Supervisors have access to everything
            return current_user
            
        if user_role in self.allowed_roles:
            return current_user
            
        # Specific sub-supervisor hierarchical override: if endpoint allows "employee", sub-supervisor can view too
